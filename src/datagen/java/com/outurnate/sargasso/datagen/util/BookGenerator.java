...
public class BookGenerator {
	private static final StringSplitter splitter = new StringSplitter((codepoint, style) -> codepoint < FontWidths.WIDTHS.length ? FontWidths.WIDTHS[codepoint] : 6);
	...
	public void pushLines(String text) {
		...
		for (String line : splitter.splitLines(text, 114, Style.EMPTY).stream().map(FormattedText::getString)
				.toList()) {
			if (lastPage.size() >= 14) {
